def SQL_FIRST_func(environment):
    knowledge_table=f"knowledge_table{environment}"
    product_mateview=f"product_mateview{environment}"
    environment_table=f"environment_table{environment}"
    category_mateview=f"category_mateview{environment}"
    reference_mateview=f"reference_mateview{environment}"
    division_table=f"division_table{environment}"
    ts_system_table=f"ts_system_table{environment}"
    return f"""WITH SearchTable(knowledge_id, error_message, title, detail, cloud_service, product_name) AS (
...
def SQL_SECOND_func(environment):
    knowledge_table=f"knowledge_table{environment}"
    product_mateview=f"product_mateview{environment}"
    environment_table=f"environment_table{environment}"
    category_mateview=f"category_mateview{environment}"
...
# product_mateview="product_mateview"
# environment_table="environment_table"
# category_mateview="category_mateview"
# reference_mateview="reference_mateview"
# division_table="division_table"
# ts_system_table="ts_system_table"

# SQL_FIRST=f"""WITH SearchTable(knowledge_id, error_message, title, detail, cloud_service, product_name) AS (
# SELECT {knowledge_table}.knowledge_id, {knowledge_table}.error_message, {knowledge_table}.title, {knowledge_table}.detail, 
# {product_mateview}.cloud_service, {product_mateview}.product_name 
# FROM {knowledge_table} 
# LEFT OUTER JOIN {product_mateview} ON {knowledge_table}.knowledge_id = {product_mateview}.knowledge_id), 
